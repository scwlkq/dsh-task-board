...
import { ApiProxyTaskBoardSessionGateway } from './host/session-apiproxy.ts'
import { TaskBoardService, type Config as TaskBoardConfig } from './host/service.ts'
 ...
 */
export async function* createTaskBoardComposition(
  ctx: TaskBoardCompositionContext,
  config: Config,
): AsyncGenerator<() => void | Promise<void>, void> {
...
export async function apply(ctx: Context, config: Config): Promise<void> {
  await ctx.effect(
    () => createTaskBoardComposition(ctx, config),
    'task-board.community.composition',
  )
}
