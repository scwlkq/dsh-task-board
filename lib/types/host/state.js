...
function freezeTask(task) {
    ...
    if (snapshot === undefined)
        throw stateError('task is not losslessly JSON serializable');
    return deepFreeze(snapshot);
}
function freezeSnapshot(snapshot) {
    const value = snapshotJsonValue(snapshot);
    if (value === undefined)
        throw stateError('snapshot is not losslessly JSON serializable');
...
function replaceLatestRound(task, round) {
    ...
    if (task.rounds.length === 0)
        throw stateError('task has no execution round');
...
function formatPosition(value) {
    ...
    if (!Number.isSafeInteger(value) || value < 1)
        throw stateError('position must be a positive safe integer');
...
function positionBetween(previous, next) {
    ...
    if (previousValue === undefined || (next !== undefined && nextValue === undefined))
        return undefined;
    if (nextValue === undefined) {
        if (previousValue > Number.MAX_SAFE_INTEGER - POSITION_STEP)
            return undefined;
 ...
 */
export function createTaskRecord(request, metadata) {
    if (metadata.automaticTitleMaxChars < 1)
        throw stateError('automatic title limit must be positive');
 ...
 */
export function editTaskRecord(task, patch, metadata, automaticTitleMaxChars) {
    if (task.status === 'running')
        throw stateError('invalid transition: cannot edit from running');
    ...
    if (patch.title !== undefined) {
        ...
        if (next.titleMode !== 'manual')
            next = { ...next, titleMode: 'manual' };
    }
    if (patch.resetAutomaticTitle === true) {
        ...
        assign('title', title);
        if (next.titleMode !== 'automatic')
            next = { ...next, titleMode: 'automatic' };
    }
    if (patch.description !== undefined)
        assign('description', patch.description);
    if (patch.acceptanceCriteria !== undefined)
        assign('acceptanceCriteria', patch.acceptanceCriteria);
 ...
 */
export function startRoundRecord(task, input) {
 ...
 */
export function appendPromptRecord(task, prompt, metadata) {
 ...
 */
export function markRoundRunningRecord(task, metadata) {
 ...
 */
export function failRoundAdmissionRecord(task, failure, metadata) {
 ...
 */
export function recordRoundEvidence(task, evidence, now) {
    ...
    const prompts = round.prompts.map((prompt) => {
        ...
        if (prompt.messageSeq === matched.messageSeq
            ...
            && prompt.turnEndSeq === matched.turnEndSeq)
            return prompt;
        return {
            ...prompt,
    ...
    if (prompts.every((prompt, index) => prompt === round.prompts[index]))
        return task;
 ...
 */
export function rollbackPromptRecord(task, promptId, now) {
    requireTransition(task, 'running', 'rollback prompt');
    const round = activeRound(task);
    if (!round.prompts.some(prompt => prompt.id === promptId))
        return task;
    return nextTask(task, now, {
        rounds: replaceLatestRound(task, {
            ...round,
 ...
 */
export function requestStopRecord(task, metadata) {
 ...
 */
export function rejectTaskRecord(task, input) {
    requireTransition(task, 'review', 'reject');
    if (input.feedback.trim().length === 0)
        throw stateError('rejection feedback is blank');
    if (task.currentSessionId === undefined)
        throw stateError('review task has no current Session');
 ...
 */
export function retryTaskRecord(task, input) {
    requireTransition(task, 'failed', 'retry');
 ...
 */
export function projectRoundOutcome(signals) {
 ...
 */
export function reconcileRound(task, projection, metadata) {
 ...
 */
export function approveTaskRecord(task, metadata) {
 ...
 */
export function reopenTaskRecord(task, metadata) {
 ...
 */
export function reorderTaskRecord(task, siblings, request, metadata) {
    ...
    if (matching.length !== 1)
        throw stateError('reorder requires the task exactly once');
    ...
    if (insertAt < 0)
        throw stateError('reorder anchor is not in the same status');
    ...
    if (directPosition !== undefined) {
        if (directPosition === task.position)
    ...
    if (moved === undefined)
        throw stateError('reordered task disappeared');
 ...
 */
export function deleteAllowed(task, confirmed) {
    switch (task.status) {
        case 'initialized':
            return true;
        case 'review':
 ...
 */
export function snapshotBoard(tasks, boardRevision) {
 ...
 */
export function composeInitialPrompt(task) {
