...
import { TaskBoardSessionRequestId, } from "./session-types.js";
import { Remote, TypertRemoteService } from '@deepseek-ai/dsh-typert-protocol';
import { taskBoardDomainSpec } from "./spec.js";
import { approveTaskRecord, appendPromptRecord, composeInitialPrompt, createTaskRecord, deleteAllowed, editTaskRecord, failRoundAdmissionRecord, markRoundRunningRecord, projectRoundOutcome, reconcileRound, recordRoundEvidence, rejectTaskRecord, requestStopRecord, reopenTaskRecord, reorderTaskRecord, retryTaskRecord, rollbackPromptRecord, snapshotBoard, snapshotTask, startRoundRecord, TaskBoardStateError, } from "./state.js";
export { taskBoardActivitySchema, taskBoardDomainSpec, taskBoardFailureSchema, taskBoardGlobalSchema, taskBoardRoundPromptSchema, taskBoardRoundSchema, taskBoardTaskSchema, } from "./spec.js";
...
function initialPosition(sequence) {
    ...
    if (!Number.isSafeInteger(value))
        throw new Error('task-board: sequence exceeds ordering range');
...
let TaskBoardService = (() => {
    ...
    return class TaskBoardService extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _followup_decorators, { kind: "method", name: "followup", static: false, private: false, access: { has: obj => "followup" in obj, get: obj => obj.followup }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        async [Service.init]() {
            ...
            this.ctx.on('agent/status', ({ agent, status }) => {
                if (status === 'idle')
                    this.scheduleReconcile(agent.id);
            }, { global: true });
            ...
            for (const activeSessionId of this.activeSessions.keys())
                this.scheduleReconcile(activeSessionId);
        }
        /**
        ...
        edit(ref, patch) {
            ...
            if (!validPatch.ok)
                return Promise.resolve(validPatch);
            return this.enqueueTask(ref.id, () => this.enqueueBoard(async () => {
                ...
                let next;
                try {
                    next = editTaskRecord(current.value, patch, { activityId: activityId(), now: Date.now() }, this.config.automaticTitleMaxChars);
                }
                catch (error) {
                    if (!(error instanceof TaskBoardStateError))
                        throw error;
                    return this.invalidRequest('patch', error.message);
                }
                if (next === current.value)
                    return success(snapshotTask(current.value));
        ...
        reorder(ref, request) {
            return this.enqueueTask(ref.id, () => this.enqueueBoard(async () => {
                ...
                for (const changed of result.changed)
                    await this.commitTask(changed, 'updated');
                const committed = this.requireTasks().get(ref.id);
                if (committed === undefined)
                    throw new Error(`task-board: reordered task '${ref.id}' disappeared`);
        ...
        followup(ref, request) {
            ...
            return this.enqueueTask(ref.id, async () => {
                ...
                try {
                    ...
                    if (!response.ok)
                        failure = this.rpcFailure('prompt-admission', response.error);
                }
                catch {
                    failure = {
        ...
        delete(ref) {
            return this.enqueueTask(ref.id, () => this.enqueueBoard(async () => {
                ...
                if (!deleted)
                    throw new Error(`task-board: committed task '${ref.id}' disappeared before deletion`);
                this.emitChange({ boardRevision, operation: 'deleted', taskId: ref.id });
                this.removeActiveSession(ref.id);
                return success({ deleted: true, taskId: ref.id });
            }, true));
        }
        /**
        ...
        resolveRef(ref) {
            ...
            if (current === undefined)
                return rejected({ code: 'task-not-found', taskId: ref.id });
        ...
        async admitPreparedRound(id, options) {
            ...
            const content = [{ type: 'text', text: options.promptText }];
            ...
            if (running !== task)
                await this.persistUpdatedTask(running);
        ...
        scheduleReconcile(currentSessionId) {
            ...
            if (id === undefined || !this.mutationAdmissionOpen)
                return;
            void this.enqueueTask(id, () => this.reconcileTaskNow(id)).catch((error) => {
        ...
        async reconcileTaskNow(id) {
            ...
            if (task === undefined || task.status !== 'running' || task.currentSessionId === undefined)
                return;
            let events;
            try {
            ...
            if (projection.outcome.kind === 'running')
                return;
            ...
            for (const event of events) {
                switch (event.type) {
                    ...
                    case 'turn/end':
                        ...
                        if (activeTurn?.turn === event.data.turn)
        ...
        requireTask(id) {
            const task = this.requireTasks().get(id);
            if (task === undefined)
                throw new Error(`task-board: task '${id}' disappeared`);
            return task;
        }
        getRequiredTask(id) {
        ...
        enqueueTask(id, operation) {
            ...
            void settled.then(() => {
                if (this.taskTails.get(id) === settled)
        ...
        requireGlobal() {
            if (this.global === undefined)
                throw new Error('task-board: service is not initialized');
            return this.global;
        }
        requireTasks() {
