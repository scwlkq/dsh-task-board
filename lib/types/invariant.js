...
function validateTask(task, fail) {
    ...
    if (activityIds.size !== task.activity.length)
        fail(`task '${task.id}' contains duplicate activity ids`);
}
function validateAll(tasks, fail) {
...
function validateChange(ctx, change, fail) {
